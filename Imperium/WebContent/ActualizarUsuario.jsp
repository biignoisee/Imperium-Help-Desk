...
<head>
	...
	<title>Actualizar Usuario - Imperium HelpDesk</title>
...
cada icono con esa nomenclatura ======= -->
  ...
  <main id="main" class="main bg-light mt-2 pl-5 pr-5">
  	<div class="container-fluid mt-5 mb-5 pt-3 pb-3 pl-4 pr-4 bg-white">
		<a href="ControladorUsuario?accion=listar" class="btn btn-danger text-white p-2 text-decoration-none" style="cursor:pointer;"><i class="bi bi-door-open-fill"></i> Volver al Lista Usuario</a>
		<h1 class="mt-3">Actualizar Usuario</h1>
		<p>Desde esta ventana podrá actualizar datos del Usuario</p>
		
		
		<form action="ControladorUsuario" class="row" method="get">
		<input type = "hidden" name = "id" value = "<c:out value = "${id}"/>">
		  ...
		  <div class="form-group col-sm-6">
		    ...
		    <input type = "text" class="form-control" name = "username" value = "<c:out value = "${username}"/>"/>
		  </div>
		  
		  <div class="form-group col-sm-6">
		    <label for="titulo">Apellido Paterno</label>
		    <input type = "text" class="form-control" name = "apellido_paterno" value = "<c:out value = "${apellido_paterno}"/>"/>
		  </div>
		  
		  <div class="form-group col-sm-6">
		    <label for="titulo">Apellido Materno</label>
		    <input type = "text" class="form-control" name = "apellido_materno" value = "<c:out value = "${apellido_materno}"/>"/>
		  </div>
		  
		  <div class="form-group col-sm-4">
		    <label for="titulo">DNI</label>
		    <input type = "text" class="form-control" name = "dni" value = "<c:out value = "${dni}"/>"/>
		  </div>
		  
		  <div class="form-group col-sm-4">
		    <label for="titulo">Contraseña</label>
		    <input type = "text" class="form-control" name = "password" value = "<c:out value = "${password}"/>"/>
		  </div>
		  		  
		  <div class="form-group col-sm-4">
		    <label for="titulo">Rol ID</label>
		    <input type = "text" class="form-control" name = "role_id" value = "<c:out value = "${role_id}"/>"/>
		  </div>
		  
		  <button type="submit" class="btn btn-primary col-sm-4 ml-3"  name="accion" value="Actualizar"><i class="fa-solid fa-floppy-disk"></i> Actualizar Usuario</button>
		</form>
	</div>
